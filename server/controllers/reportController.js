...
const getAppBranding = (user) => {
  ...
  if (email.endsWith('@speedmart.com') || email.includes('speedmart')) {
    return {
      name: 'Speedmart',
      title: 'Speedmart - Attendance Report',
      footer: 'Speedmart. All Rights Reserved.',
...
exports.exportExcel = async (req, res) => {
  ...
  try {
    ...
    // Headers
    const headers = ['S.No', 'Date', 'Time', 'Student Name', 'Roll Number', 'Department'];
...
exports.exportPDF = async (req, res) => {
  ...
  try {
    ...
    logs.forEach((log, index) => {
      ...
      if (index % 2 === 1) {
        doc.rect(40, currentY, 515, 18).fill('#F3F4F6');
    ...
    for (let i = 0; i < pages.count; i++) {
      ...
      doc.fillColor('#9CA3AF')
         .fontSize(8)
         .text(
           `Page ${i + 1} of ${pages.count} | ${branding.footer}`,
           40,
           800,
