...
exports.getAttendanceLogs = async (req, res) => {
  const { date, department, search, teacherId } = req.query;
...
exports.getStats = async (req, res) => {
  try {
    ...
    if (totalClassesToday > 0) {
      // Total present entries today (each representing attendance in a session)
      presentToday = await Attendance.countDocuments({
        date: dateStr,
        status: 'Present',
      });
      absentToday = (totalStudents * totalClassesToday) - presentToday;
      attendanceRate = (presentToday / (totalStudents * totalClassesToday)) * 100;
    }
    ...
    for (const dept of departments) {
      ...
      if (totalClassesToday > 0) {
        deptPresent = await Attendance.countDocuments({
          date: dateStr,
          student: { $in: deptStudents },
          status: 'Present',
        });
        deptTotalCount = deptTotal * totalClassesToday;
        deptAbsent = deptTotalCount - deptPresent;
      }
    ...
    if (totalClassesToday > 0) {
      for (const student of allStudents) {
        ...
        if (attendedCount < totalClassesToday) {
          absentStudents.push({
            _id: student._id,
            name: student.name,
            ...
            email: student.email,
            missedCount: totalClassesToday - attendedCount,
          });
        }
      }
    }
...
exports.getLowAttendance = async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, count: lowAttendanceStudents.length, students: lowAttendanceStudents });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
