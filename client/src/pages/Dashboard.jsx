...
const Dashboard = ({ user }) => {
  ...
  // Total Attendance states
  const [showWarnModal, setShowWarnModal] = useState(false);
  const [threshold, setThreshold] = useState(75);
  const [totalStudentsAll, setTotalStudentsAll] = useState([]);
        ...
        <div className="flex items-center space-x-3 shrink-0">
          {user?.role === 'admin' && (
            <button
              ...
              className="flex items-center space-x-2 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 bg-gradient-to-r from-indigo-650 to-indigo-600 hover:from-indigo-600 hover:to-indigo-500 text-white font-bold rounded-xl shadow-lg transition-all text-sm cursor-pointer"
            >
            ...
            <div className="pt-4 border-t border-slate-800 flex justify-end">
               <div className="flex space-x-2">
                 <button
                   ...
                   className="px-4 py-2.5 bg-blue-600 hover:bg-blue-500 text-white font-semibold rounded-xl transition cursor-pointer text-sm"
                 >
                   Refresh
                 </button>
                 <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm animate-fadeIn">
          <div className="w-full max-w-2xl glass-panel rounded-2xl border border-slate-800 shadow-2xl p-6 relative flex flex-col max-h-[85vh] animate-zoomIn">
            ...
            <div className="flex-1 overflow-y-auto py-4 min-h-[250px] pr-1">
              ...
              ) : totalStudentsAll.length > 0 ? (
                <div className="divide-y divide-slate-800/40">
                  {totalStudentsAll.map((student) => (
                    <div key={student._id} className="py-3.5 flex items-center justify-between hover:bg-slate-900/20 px-2 rounded-xl transition">
                      ...
                      <div className="text-right">
                        <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold ${student.attendanceRate < threshold ? 'bg-red-500/10 border border-red-500/20 text-red-400' : 'bg-green-500/10 border border-green-500/20 text-green-400'}`}>
                          {student.attendanceRate}%
                        </span>
                        <p className="text-[10px] text-slate-500 mt-1 font-semibold">{student.presentCount} / {student.totalDays} Classes</p>
                      </div>
                    </div>
                  ))}
                </div>
